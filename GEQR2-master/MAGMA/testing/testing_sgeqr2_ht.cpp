...
int main( int argc, char** argv)
{
    ...
    magma_int_t M, N, n2, lda, ldda, lwork, info, min_mn, cnt_nan, cnt_inf, nan_info;
    magma_int_t ione     = 1;
...
/*	   for(int i=0; i<(M*N); i++)
	    	 h_A[i] = h_R[i] = rand()%5 + 1;
	
	   for(int i=0; i<(min_mn); i++)
	    	 tau[i] = 1;
*/	   
           magma_ssetmatrix( M, N, h_R, lda, d_A, ldda, opts.queue );
           magma_ssetvector( min_mn, tau, 1, dtau, 1, opts.queue );
...
//            magma_sgeqr2_gpu( M, N, d_A, ldda, dtau, dwork, opts.queue, &info );
            sgeqr2_HT( opts.handle, M, N, d_A, ldda, dtau, dwork, opts.queue, &info ); 
...
//            nan_info = magma_snan_inf_gpu(MagmaFull, M, N, d_A, ldda, &cnt_nan, &cnt_inf);
